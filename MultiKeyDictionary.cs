...
namespace CLP
{
	public class MultiKeyDictionary<K, L, V>
	{
		...
		public void IncrementValue(K primaryKey, L subKey)
		{
			readerWriterLock.EnterWriteLock();

			try
			{
				baseDictionary[primaryKey] += 1;
			}
			finally
			{
				readerWriterLock.ExitWriteLock();
		...
		public void EditValue(K primaryKey, L subKey, V val)
		{
			readerWriterLock.EnterWriteLock();

			try
			{
				baseDictionary[primaryKey] = val;
			}
			finally
			{
				readerWriterLock.ExitWriteLock();
